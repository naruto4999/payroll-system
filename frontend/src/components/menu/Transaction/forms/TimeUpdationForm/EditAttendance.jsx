...
import {
	...
	DirectMachineAttendanceSchema,
	MdbMachineAttendanceSchema,
} from './TimeUpdationSchema';
import {
...
const EditAttendance = ({
	...
	const { values, errors, touched, handleChange, handleSubmit, setFieldValue, isSubmitting, isValid } =
		useFormikContext();
	...
	const [showMdbMachineAttendanceModal, setShowMdbMachineAttendanceModal] = useState(false);
	const [showDirectMachineAttendanceModal, setShowDirectMachineAttendanceModal] = useState(false);
	...
	useEffect(() => {
		...
		const performCalculations = () => {
			if (currentEmployeeProfessionalDetail && currentEmployeeSalaryDetail) {
				const dailyWageEmployee = currentEmployeeSalaryDetail.salaryMode.toLowerCase() === 'daily';
				// let extraOff = null;
				...
				for (const day in values.attendance) {
					...
					if (!values.attendance[day].manualMode) {
						...
						if (!dailyWageEmployee) {
							//Checking for weekly off (daily wage employees don't get WO or HD or WO* or HD*)
						...
						if (!dailyWageEmployee) {
							//Cheking for extra off (daily wage employees don't get WO or HD or WO* or HD*)
							if (
								!holidayDay &&
								memoizedExtraOffDate &&
								parseInt(day) == memoizedExtraOffDate.getDate()
							) {
								weeklyOffDay = true;
	...
	useEffect(() => {
		if (!isSubmitting && employeeAttendance && currentEmployeeProfessionalDetail && updateEmployeeId) {
			...
			for (let day = 1; day <= daysInMonth; day++) {
				...
				let weeklyOffDay = false; //variable for both weekly off and extra off
				let holidayDay = false;

				//Daily wage employees don't get weekly off or holiday off
				if (!dailyWageEmployee) {
					//Means it's holiday off for employee
					for (const holiday of holidays) {
						const holidayDate = new Date(holiday.date);
					...
					//Means it's weekly off for employee
					if (!holidayDay && attendanceWeekday === weeklyOffIndex) {
					...
					//Means it's extra off for employee
	...
	const machineAttendance = async (formikBag) => {
		...
		formData.append('allEmployeesMachineAttendance', formikBag.allEmployeesMachineAttendance);
		try {
			...
			setShowMdbMachineAttendanceModal(false);
		} catch (err) {
	...
	useEffect(() => {
		...
		setShowLoadingBar(
			isFetchingAllEmployeeAttendance || isLoadingAllEmployeeProfessionalDetail || isAddingMachineAttendance
		);
	}, [isFetchingAllEmployeeAttendance, isLoadingAllEmployeeProfessionalDetail, isAddingMachineAttendance]);
	...
	return (
		<>
			...
			<form action="" className="w-fit" onSubmit={handleSubmit}>
				...
				{/* Render the buttons and form fields using a React Portal */}

				{document.getElementById('buttons-portal-destination') != null &&
					ReactDOM.createPortal(
						...
						document.getElementById('buttons-portal-destination') // Portaling into the destination div
					)}

				{/*Submit and Update Button*/}
				{document.getElementById('update-buttons-portal-destination') != null &&
					ReactDOM.createPortal(
						<>
							<button
								className={classNames(
						...
						document.getElementById('update-buttons-portal-destination')
					)}
			</form>
			...
			<ReactModal
				...
				onRequestClose={() => setShowMdbMachineAttendanceModal(false || isAddingMachineAttendance)}
				style={{
					overlay: {
						backgroundColor: 'rgba(0, 0, 0, 0.75)',
						zIndex: 20,
			...
			>
				...
				>
					{() => (
						<MdbMachineAttendance
							setShowMdbMachineAttendanceModal={setShowMdbMachineAttendanceModal}
							month={values.month}
							year={values.year}
							manualFromDate={values.manualFromDate}
							manualToDate={values.manualToDate}
							updateEmployeeId={updateEmployeeId}
						/>
					)}
				</Formik>
			...
			<ReactModal
				className="items-left z-100 fixed inset-0 mx-2 my-auto flex h-fit flex-col gap-4 rounded bg-zinc-300 p-4 shadow-xl dark:bg-zinc-800 sm:mx-auto sm:max-w-lg"
				isOpen={showDirectMachineAttendanceModal}
				onRequestClose={() => setShowDirectMachineAttendanceModal(false || isAddingMachineAttendance)}
				style={{
					overlay: {
						backgroundColor: 'rgba(0, 0, 0, 0.75)',
						zIndex: 20,
			...
			>
				<Formik
					...
					validationSchema={DirectMachineAttendanceSchema}
					onSubmit={machineAttendance}
				>
					{() => (
						<DirectMachineAttendanceModal
							setShowDirectMachineAttendanceModal={setShowDirectMachineAttendanceModal}
							month={values.month}
							year={values.year}
							manualFromDate={values.manualFromDate}
							manualToDate={values.manualToDate}
