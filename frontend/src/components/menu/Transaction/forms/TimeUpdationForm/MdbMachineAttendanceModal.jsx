...
const MdbMachineAttendance = ({
	...
	if (
		manualFromDate == '' ||
		manualFromDate == null ||
		manualFromDate == undefined ||
		manualToDate == '' ||
		manualToDate == null ||
		manualToDate == undefined
	) {
		return (
			<div className="w-full">
				<div className="mx-auto w-fit text-base font-bold text-red-500 dark:text-red-700">
					Please Enter From and To Date first
				</div>
			</div>
		);
		...
		new Date(Date.UTC(year, month - 1, manualFromDate)) > new Date(Date.UTC(year, month - 1, manualToDate))
	) {
		return (
			<div className="w-full">
				<div className="mx-auto w-fit text-base font-bold text-red-500 dark:text-red-700">
		...
		return (
			<div className="text-gray-900 dark:text-slate-100">
				...
				<h3 className="mb-2 text-sm font-medium text-black text-opacity-100 dark:text-white dark:text-opacity-70">
					Attendance Period:{' '}
					{
						<span className="font-bold text-amber-500">
							{formatDate(new Date(Date.UTC(year, month - 1, manualFromDate)))}
						</span>
					}{' '}
					To{' '}
					{
						<span className="font-bold text-amber-500">
							{formatDate(new Date(Date.UTC(year, month - 1, manualToDate)))}
						</span>
					}
				</h3>
