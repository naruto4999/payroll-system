...
const AttendanceMonthDays = memo(({ day, year, month, shift, leaveGrades, otMin, lateMin, holidays }) => {
	// console.log(shift);
	...
	return (
		<div
			className={classNames(
				...
				weekdayIndex == 0 || isHoliday ? 'dark:bg-red-700 dark:bg-opacity-40' : ''
			)}
		>
			<h6
				className={classNames(
					weekdayIndex == 0 || isHoliday ? 'dark:text-red-600' : '',
			...
			<section className="flex h-full flex-row divide-x divide-dashed divide-blueAccent-600/80">
				...
				<h6 className="my-auto w-20 cursor-default pl-2 pr-2 text-xs dark:text-green-600">
					{otMin !== ''
						? `${String(Math.floor(otMin / 60)).padStart(2, '0')}:${String(otMin % 60).padStart(2, '0')}`
						: ''}
				</h6>
