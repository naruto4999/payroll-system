...
const EditSalary = React.memo(
    ({
        ...
        const data = useMemo(
            () => (singleEmployeeSalaryEarning ? [...singleEmployeeSalaryEarning] : []),
        ...
        useEffect(() => {
            ...
            if (prevSingleEmployeeSalaryEarningRef.current !== singleEmployeeSalaryEarning) {
                if (isSingleEmployeeSalaryEarningSuccess == true && data.length != 0) {
                    const initialValues = {
                        year: values.year,
                        earnings: {},
                        sameValue: {
                    ...
                    fetchedEarningsHeads.forEach((item) => {
                        ...
                        months.forEach((month) => {
                            singleEmployeeSalaryEarning.forEach((salaryEarning) => {
                                if (item.name === salaryEarning.earningsHead.name) {
                                    let fromDate = new Date(salaryEarning.fromDate);
                                    let toDate = new Date(salaryEarning.toDate);
                                    let formik_field_date_representation = new Date(`${values.year}-${month}-01`);
                                    if (isDateWithinRange(formik_field_date_representation, fromDate, toDate)) {
                                        setFieldValue(`earnings.${item.name}.${month}`, salaryEarning.value);
                                    } else if (
                                        formik_field_date_representation <
                                        new Date(`${yearOfJoining}-${monthOfJoining}-01`)
                                    ) {
                                        setFieldValue(`earnings.${item.name}.${month}`, '');
                                    }
                                }
                            });
                        });
            ...
            prevSingleEmployeeSalaryEarningRef.current = singleEmployeeSalaryEarning;
        }, [singleEmployeeSalaryEarning]);
        ...
        useEffect(() => {
            ...
            for (const earningHead in values.earnings) {
                ...
                if (sameValueMonths.length > 1) {
                    const selectedMonthValue = values.earnings[earningHead][sameValueMonths[0]];
                    sameValueMonths.forEach((month) => {
                        setFieldValue(`earnings.${earningHead}.${month}`, selectedMonthValue);
                    });
                }
            }
        }, [values.sameValue]);
                                    ...
                                    <tr>
                                        ...
                                        {Object.keys(values?.earnings).map((key) => (
                                            <th className="w-fit" key={key}>
                                                <h3 className="text-base text-blueAccent-600 dark:text-blueAccent-500 md:text-xl">
                                                    <span className="block md:hidden">{key.slice(0, 5)}</span>
                                                    <span className="hidden md:block">{key}</span>
                                                </h3>
                                            </th>
                                        ))}
                                        <th>
                                            <h3 className="text-base text-green-800 dark:text-green-500 md:text-xl">
                                                Total
                                            </h3>
                                        </th>
                                        <th>
                                            <h3 className="text-base text-amber-700 dark:text-amber-600 md:text-xl">
                                                Same
                                            </h3>
                                        </th>
                                    </tr>
                                                        ...
                                                        <td key={key} className={classNames('')}>
                                                            <Field
                                                                className={classNames(
                                                                    ...
                                                                    'custom-number-input mt-1 w-full rounded border bg-zinc-50   bg-opacity-50 text-sm outline-none transition-opacity focus:border-opacity-100 dark:bg-zinc-700 dark:focus:border-opacity-75 dark:group-hover:bg-zinc-800 md:p-1 md:text-base'
                                                                )}
                                                                type="number"
                                                                name={`earnings.${key}.${month}`}
                                                                disabled={
                                                                    values.year == yearOfJoining &&
                                                                    parseInt(month) < monthOfJoining
                                                                }
                                                            />
                                                            <div className="mt-1 text-xs font-bold text-red-500 dark:text-red-700">
                                                                <ErrorMessage name={`earnings.${key}.${month}`} />
                                                            </div>
                                                        </td>
                                                    );
                                                })}
                                                <td
                                                    className="relative border-slate-500  bg-opacity-50 p-1 text-green-600 md:my-2 md:px-4 "
                                                    key={index}
                                                >
                                                    {totalEarnings}
                                                </td>
                                                {/* h-44 beause Other input fields are 44 too */}
                                                <td className="h-[30px]  rounded-r-lg md:h-11">
