...
const SalaryOvertimeSheet = () => {
	...
	const data = useMemo(() => {
		...
		const filteredData = employeePersonalDetails.filter((employee) => {
			if (employee.dateOfJoining) {
				if (filterYearlyAdvanceEmployees == true) {
					const isYearlyAdvanceEmployee = employeesForYearlyAdvanceReport?.employeeIds?.some(
						(id) => id === employee.id
					);
		...
		selectedDate,
		employeePreparedSalaries,
		filterYearlyAdvanceEmployees,
	...
	useEffect(() => {
		setShowLoadingBar(
			isLoadingEmployeePersonalDetails ||
				isLoadingEmployeesForYearlyAdvanceReport ||
				isFetchingEmployeesForYearlyAdvanceReport
		);
	}, [
		isLoadingEmployeePersonalDetails,
		isLoadingEmployeesForYearlyAdvanceReport,
					...
					<div className="mt-4">
						<Formik
							...
							component={(props) => (
								<FilterOptions
									{...props}
									setFilterYearlyAdvanceEmployees={setFilterYearlyAdvanceEmployees}
								/>
							)}
						/>
					</div>
