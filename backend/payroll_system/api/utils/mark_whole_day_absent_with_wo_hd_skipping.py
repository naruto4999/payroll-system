...
def mark_whole_day_absent_with_wo_hd_skipping(from_date, to_date, employee, company_id, user):
    """
    ...
    for day in filtered_attendance_records:
        ...
        if day.first_half.name=='WO' and day.second_half.name=='WO':
            ...
            if paid_days_count_for_past_six_days(day.date, company_id=company_id, user=user, employee=employee)==((weekly_off_holiday_off.min_days_for_weekly_off * 2)): #paid days count should be at least a whole Working day (P-P) more
    ...
    if converted_halves != 4 and converted_halves!= 0:
        raise ValueError("Converted halves must 4 or 0")
    elif converted_halves>=4:
