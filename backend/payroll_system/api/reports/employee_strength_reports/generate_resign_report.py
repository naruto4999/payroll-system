...
class FPDF(FPDF):
    ...
    def header(self):
        ...
        self.cell(w=None, h=6, text=f"Resign Report for the period of {self.from_date.strftime(f'%e{day_suffix} %B, %Y')} to {self.to_date.strftime(f'%e{day_suffix} %B, %Y')}", align="L", new_x="LMARGIN", new_y='NEXT', border=0)
...
def generate_resign_report(user, request_data, employees):
